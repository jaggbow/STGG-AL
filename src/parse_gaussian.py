...
if __name__ == "__main__":
    ...
    parser.add_argument("--labeling_csv_path", type=str)
    parser.add_argument("--gaussian_dir", type=str)
    args = parser.parse_args()
   
    csv_path = Path(args.labeling_csv_path)
    smiles_path = Path(args.smiles_path)
    smiles_id = smiles_path.stem
    df_labeling = pd.read_csv(csv_path)
    ...
    youhou = 0
    for lg_path in gaussian_dir.glob("*.log"):
        n += 1
        if lg_path.exists():
            try:
                ...
                if vs1 is None or vdelta is None:
                    continue
                result["molecule_id"].append(name)
                result["SMILES"].append(df_labeling[df_labeling["molecule_id"] == name]["SMILES"].item())
                result["vs1_gt"].append(vs1)
                result["vdelta_gt"].append(vdelta)
                ...
                c += 1
                if (vs1 is not None) and (vdelta is not None) and (vs1 > 2.6) and (vdelta < 0.3):
                    youhou += 1
