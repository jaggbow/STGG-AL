...
TORCH_NCCL_BLOCKING_WAIT=0 CUDA_VISIBLE_DEVICES=0 python train_condgenerator.py --dataset_name xtb --num_layers 3 --tag exp_xtb_finetune_f_osc --bf16 \
...
--max_len 700 --gpt --no_bias --rmsnorm --rotary --no_test_step --limited_properties --xtb_reward_type f_osc \
...
ckpt_finetune="exp_xtb_finetune_f_osc"

# Hyperparameters (Can be modified)
tag="active_learning_fosc"
...
--limited_properties --xtb_reward_type f_osc"
only_training_args=""
sampling_args="--temperature_min 0.6 --temperature_max 1.1 --guidance_min 0.5 --guidance_max 1.5 \
--last --not_allow_empty_bond \
--max_number_of_atoms 70 --max_ring_size 6 --remove_duplicates \
--test --specific_conditioning \
--sample_batch_size 250 --num_samples 2000 --best_out_of_k 1 --specific_mae_props f_osc \
--xtb_reward_type f_osc --top_k_to_add 999999999 --specific_minmax 0 1 --top_k_output 100" # For each specific_values, specify if they must be minimized (-1), maximized (1), or run the L2 MAE (0)
min_reward=6.0
max_reward=11.0
cond_args_min="666 ${min_reward}" # note: please copy this further below if using adaptive rewards
cond_args_max="666 ${max_reward}" # note: please copy this further below if using adaptive rewards
...
# 666 means masked (for this task, we generate conditional on f_osc in Uniform([min_reward, max_reward]); there is no conditioning value for wavelength hence we mask it)
cond_args_min="666 ${min_reward}" 
cond_args_max="666 ${max_reward}"
fi
